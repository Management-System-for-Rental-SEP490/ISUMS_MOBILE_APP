/**
 * SSE notification stream — đọc `EXPO_PUBLIC_NOTIFICATION_STREAM_URL`, Bearer token giống REST.
 * 401 sau refresh → logout (không retry SSE). 403 → toast một lần + fatal permanent.
 * Dòng data malformed liên tiếp (ngưỡng env) → fatal; lẻ tẻ thì skip, không đóng socket.
 */
