...
export type StackScreenTitleBadgeProps = {
  ...
  /**
   * `onBrand`: trên nền gradient giống `<Header />` (mặc định).
   * `onLight`: nền sáng (màn đặc biệt không dùng `StackScreenTitleHeaderStrip`).
   */
  tone?: "onBrand" | "onLight";
};

/**
 * Vùng trên cùng giống header: gradient thương hiệu + safe area + status bar chữ sáng
 * để tách khỏi nền body trắng (thanh tiện ích không “chìm” vào một khối trắng).
 */
...
/** Hàng back | tiêu đề | nút phải / balance — đặt con trực tiếp trong `StackScreenTitleHeaderStrip`. */
...
/** Nút back (hoặc +) trên nền gradient: vòng mờ sáng, icon dùng `stackScreenTitleOnBrandIconColor`. */
