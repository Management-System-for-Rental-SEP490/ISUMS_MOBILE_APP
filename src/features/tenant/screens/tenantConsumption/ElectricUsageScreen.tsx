...
const ElectricUsageScreen = () => {
  ...
  const alerts = useTenantAlerts(houseId);
    ...
    >
      ...
      {latestAlert ? (
        <View style={styles.alertBanner}>
          <Text style={styles.alertBannerTitle}>{latestAlert.title}</Text>
          <Text style={styles.alertBannerText}>
            {(latestAlert.areaName ?? "Toàn nhà")} • {latestAlert.level}
          </Text>
        </View>
      ) : null}
      ...
      <View style={styles.switchWrap}>
        <TouchableOpacity style={[styles.switchBtn, styles.switchBtnActive]}>
          <Text style={[styles.switchText, styles.switchTextActive]}>Điện</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.switchBtn}>
          <Text style={styles.switchText}>Nước</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.metricRow}>
        <UsageMiniCard label="Hôm nay" value={usage.dayVal} unit="kWh" />
        <UsageMiniCard label="Tuần này" value={usage.weekVal} unit="kWh" />
        <UsageMiniCard label="Tháng này" value={usage.monthVal} unit="kWh" />
      </View>
      ...
      <View style={styles.bigCard}>
        ...
        {power ? (
          <View style={styles.realtimeGrid}>
            <RealtimeStat label="Điện áp" value={`${fmt(f?.v, 0)}V`} color="#22C55E" />
            <RealtimeStat label="Dòng điện" value={`${fmt(f?.i, 2)}A`} color="#2563EB" />
            <RealtimeStat label="Công suất" value={`${fmt(f?.p, 0)}W`} color="#F97316" />
            <RealtimeStat label="Chỉ số điện kế" value={`${fmt(f?.kwh, 3)} kWh`} />
            <RealtimeStat label="Tần số" value={`${fmt(f?.hz, 0)}Hz`} />
            <RealtimeStat label="PF" value={fmt(f?.pf, 2)} />
          </View>
        ) : (
